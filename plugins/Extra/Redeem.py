...
from info import ADMINS, LOG_CHANNEL, USERNAME, MYBOT
...
<b>ᴄᴏᴘʏ ᴛʜᴇ ᴀʙᴏᴠᴇ ᴄᴏᴅᴇ ᴀɴᴅ ᴘᴀꜱᴛᴇ ᴛᴏ ᴛʜᴇ ʙᴏᴛ ᴊᴜꜱᴛ ɪᴛ🔥</b>"""

        keyboard = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("♻️ Redeem Here ♻️", url=MYBOT)],
                [InlineKeyboardButton("‼️ Any Query ‼️", url=USERNAME)]
            ]
        )
        ...
        if redeem_code in VALID_REDEEM_CODES:
            try:
                ...
                if seconds > 0:
                    ...
                    await message.reply_text(
                        f"<b>ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ !!\n\nᴜꜱᴇʀ: {user.mention}\nᴜꜱᴇʀ ɪᴅ: {user_id}\nᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ: <code>{time}</code>\n\nᴇxᴘɪʀʏ ᴅᴀᴛᴇ: {expiry_str_in_ist}</b>",
                        disable_web_page_preview=True
                    )
                ...
                await message.reply_text(f"<b>ᴀɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ ᴡʜɪʟᴇ ʀᴇᴅᴇᴇᴍɪɴɢ ᴛʜᴇ ᴄᴏᴅᴇ: {e}</b>")
        else:
            await message.reply_text("ɪɴᴠᴀʟɪᴅ ʀᴇᴅᴇᴇᴍ ᴄᴏᴅᴇ ᴏʀ ᴇxᴘɪʀᴇᴅ.")
    else:
        await message.reply_text("ᴜꜱᴀɢᴇ: /redeem <code>")
