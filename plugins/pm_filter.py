...
# zishan [
from database.akreferdb import referdb
...
async def refercall(bot, query):
    btn = [[
        InlineKeyboardButton('invite link', url=f'https://telegram.me/share/url?url=https://telegram.me/{bot.me.username}?start=reff_{query.from_user.id}&text=Hello%21%20Experience%20a%20bot%20that%20offers%20a%20vast%20library%20of%20unlimited%20movies%20and%20series.%20%F0%9F%98%83'),
...
async def next_page(bot, query):
    ...
    if settings["link"]:
        ...
        for file_num, file in enumerate(files, start=offset+1):
            links += f"""<b>\n\n{file_num}. <a href=https://telegram.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
    else:
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        btn = [[
                    InlineKeyboardButton('⭐ ғᴀsᴛ ᴅᴏᴡɴʟᴏᴀᴅ 🚀', url=online)
                ],[
        ...
        buttons = [[
            InlineKeyboardButton('☆📸 𝙎𝙚𝙣𝙙 𝙨𝙘𝙧𝙚𝙚𝙣𝙨𝙝𝙤𝙩 📸☆', url=USERNAME)
        ],[
    ...
    #MrAK
       ...
       await query.message.edit_media(
        ...
        buttons = [[
            InlineKeyboardButton('☎️ 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗢𝘄𝗻𝗲𝗿 𝗧𝗼 𝗞𝗻𝗼𝘄 𝗠𝗼𝗿𝗲', url=USERNAME)
        ],[
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    if not spoll:
        ...
        await searching_animation_msg.delete()
        if not files:
